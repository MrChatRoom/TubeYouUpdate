...
function sfx(freq, type="square", t=0.05, v=0.05) {
  initAudio()
  const o = audioCtx.createOscillator()
  const g = audioCtx.createGain()
...
function explode(x, y, big=false) {
  ...
  shake = big ? 18 : 10
  sfx(big ? 80 : 140, "sawtooth", 0.08, 0.08)
}
...
window.addEventListener("keydown", e => {
  ...
  if (e.code === "Space" && ship.alive && !paused) {
    ...
    escorts.forEach((_, i) => {
      ...
      const offset = 22 + Math.floor(i / 2) * 16
...
function update() {
  ...
  escorts.forEach((_, i) => {
    const side = i % 2 === 0 ? -1 : 1
    const offset = 28 + Math.floor(i / 2) * 18
